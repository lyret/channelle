...
export function restify(): restifyLib.Server {
  ...
  // Define client interface handler
  const clientInterfaceHandler = async (
    req: restifyLib.Request,
    res: restifyLib.Response,
  ) => {
    try {
  ...
  // Serve the client interface for specific routes and their nested paths
  // Use regex patterns to match exact routes
